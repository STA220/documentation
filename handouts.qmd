...
params:
  ...
  prepare_el11: false # Use to "Manually" prepare new recap version 
---
...
#| echo: false
downloads <-
  dplyr::tibble(file = dir(".", "EL.*\\.qmd")) |>
  dplyr::mutate(
    session = gsub("_site/|\\.qmd", "", file),
    PDF = sprintf("[PDF](%s.pdf)", session),
    Word = sprintf("[DOCX](%s.docx)", session)
  ) |>
  dplyr::arrange(readr::parse_number(session)) |>
  dplyr::select(-file)

if (!knitr::pandoc_to("typst")) {
  downloads |>
    gt::gt() |>
    gt::fmt_markdown(columns = c(PDF, Word))
}
```
...
# Prepary summary/recap lecture ------------------------------------------

if (params$prepare_el11) {
  # Då EL10 inte är obligatorisk skippar vi den
  files <- setdiff(files, sprintf("./EL%d.qmd", c(2:3, 10)))
  FILE <- "EL11_raw.qmd"
  APPEND <- TRUE
  fs::file_create(FILE)
  readr::write_file(
...
for (f in files) {
  ...
  # Läslista till handouts men inte recap
  if (!params$prepare_el11) {
    # Link to slides
    ...
    cat(
      paste0("- ", extract_citation_blocks(body, f), collapse = "\n"),
      "\n\n"
    )
    cat(":::\n\n")
  }
